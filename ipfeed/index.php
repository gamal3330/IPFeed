...
$defaultSettingsDir = dirname(__DIR__) . '/ip-feed-manager-private';
...
$ipsFile = (string) appConfigValue($appConfig, 'files.feed', __DIR__ . '/ips.txt');
...
require_once __DIR__ . '/app/virustotal.php';
require_once __DIR__ . '/app/ip_feed.php';
...
$webRealPath = realpath(__DIR__) ?: __DIR__;
...
$systemHealthChecks = [
    ...
    [
        'group' => 'الأمان',
        'name' => 'حماية .htaccess',
        'status' => file_exists(__DIR__ . '/.htaccess') && file_exists(__DIR__ . '/app/.htaccess') ? 'ok' : 'warning',
...
$systemHealthSummary = [
    'ok' => count(array_filter($systemHealthChecks, static fn (array $row): bool => ($row['status'] ?? '') === 'ok')),
    'warning' => count(array_filter($systemHealthChecks, static fn (array $row): bool => ($row['status'] ?? '') === 'warning')),
    'error' => count(array_filter($systemHealthChecks, static fn (array $row): bool => ($row['status'] ?? '') === 'error')),
];
